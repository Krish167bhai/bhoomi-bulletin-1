...
class SavedSearchService {
    static async checkMatchingAlerts(property) {
        try {
            const activeSearches = await prisma_js_1.prisma.savedSearch.findMany({
                where: { alertsActive: true },
                include: { user: true },
            });
            for (const search of activeSearches) {
                // Do not notify the poster of the property
                if (search.userId === property.userId)
                    continue;
                let filters = {};
                try {
                ...
                if (isMatch) {
                    ...
                    await prisma_js_1.prisma.savedSearch.update({
                        where: { id: search.id },
