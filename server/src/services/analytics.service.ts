...
export class AnalyticsService {
  ...
  static async getWebsiteOverview() {
    ...
    const pendingSubmissions = await prisma.property.count({
      where: { status: 'PENDING_APPROVAL' },
    });

    const activeAds = await prisma.property.count({
      where: { isAdvertisement: true, status: 'APPROVED' },
    });

    const expiredAds = await prisma.property.count({
      where: { isAdvertisement: true, status: 'EXPIRED' },
    });

    const totalProperties = await prisma.property.count();
    const totalArticles = await prisma.article.count();
    const totalVideos = await prisma.article.count({
      where: { category: 'VIDEO_NEWS' },
    });

    const totalRequirements = await prisma.propertyRequirement.count();
    ...
    const verifiedAgents = await prisma.user.count({
      where: {
    ...
    const totalPropertyViews = await prisma.analyticsEvent.count({
    ...
    // Group by date
    const dailyMap: Record<string, { date: string; pageViews: number; propertyViews: number; enquiries: number }> = {};
    for (let i = 6; i >= 0; i--) {
    ...
    recentEvents.forEach((ev) => {
      ...
      if (dailyMap[dayKey]) {
        if (ev.eventType === 'PAGE_VIEW') dailyMap[dayKey].pageViews++;
        else if (ev.eventType === 'PROPERTY_VIEW') dailyMap[dayKey].propertyViews++;
        else if (ev.eventType === 'ENQUIRY_CLICK') dailyMap[dayKey].enquiries++;
      }
    });
  ...
  static async getLeadAnalytics() {
    const totalLeads = (await prisma.enquiry.count()) + (await prisma.propertyRequirement.count());
    ...
    const requirementsByStatus = await prisma.propertyRequirement.groupBy({
      by: ['status'],
      _count: { id: true },
    });
  ...
  static async getUserAdAnalytics(userId: string) {
